...
# from joeynmt.training import TrainManager, train
from prediction import translate
...
class ActiveLearningLoop(object):
    ...
    def execute(self, ac_dataset, methods,
        ...
        self._check_inputs(methods, ac_dataset, train_perc,
                           test_perc, label_block_size)

        # get list of implemented acquisition functions
        acq_funcs = {a: acquisition_functions.__dict__.get(a)   # check return value, no need to change
                     for a in dir(acquisition_functions)
                     if isinstance(acquisition_functions.__dict__.get(a),
                                   types.FunctionType)}

        directory = os.path.join("../outputs/", self.name)      #
        # if directory already exists, then delete it
        if os.path.exists(directory):
            shutil.rmtree(directory)

        # considering training already done, ac will only work on ac_dataset
        ...
        for run_i in range(nb_runs):
            ...
            run_directory = os.path.join(directory, 'run_{}'.format(run_i + 1))     # keep
            ...
            while True:
                ...
                for m_id, method in enumerate(self.methods):
                    ...
                    if method['acq_func'] != 'random':
                        ...
                        hypotheses, tokens, y_pred, y_pred_attn = translate(
                        ...
                        hypotheses, tokens, y_pred, y_pred_attn = np.zeros(translate(  ## dont do prediction
                            cfg_file=args.config_path,
                            ckpt=args.ckpt,
                            output_path=args.output_path,
                        ))
    ...
    def execute_ac(self, ac_dataset,cfg_file,ckpt):
        """
        sudo code for active learning via pooling.
        for each src sentence in ac_dataset:
            get 10 best prediction loss(hopotheses) on src, y_pred
            for each method:
                get approximation on y_pred, which one should be the best, h_pred
            pick the best method based on approximation from h_pred, h_pred_best
        re-compute the pred loss, trg vs h_pred_best,

        sudo code for active learning via streaming.


        """
        df = ac_dataset.reset_index()  # make sure indexes pair with number of rows
...
if __name__ == "__main__":
    import pandas as pd
    ac_dataset = pd.read_csv("/home/ubuntu/joeynmt_kriti/test/data/datasets_enhi_25_23.04bleu/final.csv").head(10)

    ac = ActiveLearningLoop("m1")
    ...
    ac.execute_ac(ac_dataset,cfg_file,ckpt)
